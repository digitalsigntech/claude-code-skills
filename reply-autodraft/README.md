...
If `APP_CRM_DB` points at a contacts SQLite DB, the drafter also looks up each sender's name/company there — inbound mail often has no signature, and the greeting name has to come from the CRM. The looked-up name is registered with the masker so it still reaches the cloud model only as a `[[PERSON_N]]` token.
