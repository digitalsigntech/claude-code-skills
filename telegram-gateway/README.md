...
| `src/reflex_audit.py` | After a CLIP-backed reflex answers a "show me" request, the same prompt goes to the main LLM on a background thread in its own session; anything it finds that the reflex did not send is posted as a supplement (`➕ The fast search missed N items`). A complete answer stays silent. Extras obey the file reflex's own fence, so an LLM picking the files cannot widen who sees what. Logs every run to `logs/reflex_audit.jsonl`. `TG_REFLEX_AUDIT=0` to disable. |
| `src/file_reflex.py` | Optional generic file reflex: "show/fetch/get/give me <thing>" resolved against the CLIP image index and a cached workspace walk (workspace files in DMs + Always-Nemotron private groups only); sends only a full-token-coverage match (docs via `sendDocument`, images via the photo path), everything else falls through to the LLM turn. A query naming a KIND of document (manual, datasheet, diagram, SDS, template, price list …) must additionally match the hit's own filename/annotation title, not its tags — otherwise a diagram cross-referencing a manual gets sent as the manual (`photo_reflex._kind_ok`). |
