...
> This is the gateway that runs on **the box** (the the company appliance). Some features
> below (KB reflex, docpipe ingest, CLIP media search, email injection, chat
> archive) are the company-specific integrations. They **degrade
...
| `APPEND_SYSTEM` | (concise-reply prompt) | Passed via `--append-system-prompt`; biases Claude toward short, direct replies. **Customize this for your project** — it currently mentions the company specifics. |
| `TG_MAX` / `RICH_MAX` | `4000` / `32768` | Message chunk size / rich-message payload cap. |
| `KB_REFLEX` | `1` (env `TG_KB_REFLEX`) | Optional tier-1 KB quick-answer (the company-specific; set `0` to disable). |
...
## Optional integrations (the company-specific — safe to remove)

These are wired into `gateway.py`/`tgconf.py` for the the company appliance. Each is
