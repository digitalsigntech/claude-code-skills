...
token where a URL would go (the Mac dev instance's shape). It survives any parser as an
...
HOME = os.environ.get("APP_ROOT_HOME", os.path.expanduser("~"))
DB = f"{HOME}/the company/operations/reminders/reminders.db"
REALTIME = f"{HOME}/the company/voice/realtime"
MINT_URL = "http://127.0.0.1:8478/{secret}/mint-token"
SENDFILE = f"{HOME}/the company/telegram/sendfile.py"
LIMIT = 20
...
def _summarize(text):
    ...
    try:
        import sys
        sys.path.insert(0, f"{HOME}/the company/operations/reminders")
        import reminders
...
# the Mac dev instance cited, and it is NOT matched here on purpose: it never says
