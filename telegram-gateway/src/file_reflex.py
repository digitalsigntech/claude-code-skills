...
random." One verb surface, three sources, strict matching, no LLM turn:
 ...
 3. workspace files — a cached walk of the the company tree (~4k files, refreshed every
...
Claude turn — a miss costs ~10ms and hijacks nothing. Toggle: APP_FILE_REFLEX=0.
"""
...
# The object must be a noun phrase, not a question ("get me whatever the friend said" and
...
def _index():
    with _LOCK:
        if time.time() - _WALK["ts"] < WALK_TTL:
            return _WALK["files"]
    files = []
    for root, dirs, names in os.walk(C.APP_ROOT):
        dirs[:] = [d for d in dirs if d not in EXCLUDE_DIRS
                   and not d.startswith(EXCLUDE_DIR_PREFIXES)]
        rel = os.path.relpath(root, C.APP_ROOT)
...
def _send_doc(chat_id, path, t0):
    rel = os.path.relpath(path, C.APP_ROOT)
...
if __name__ == "__main__":
    ...
    if not r:
        ...
        print("-> sendDocument:", os.path.relpath(r[1], C.APP_ROOT))
    else:
