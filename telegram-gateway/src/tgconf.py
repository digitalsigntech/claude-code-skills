"""Config for the the company Telegram gateway.
...
# tools, in the the company workspace, with one persistent session per Telegram chat.
...
CLAUDE_WORKDIR = APP_ROOT
CLAUDE_MODEL = os.environ.get("APP_TG_MODEL", "claude-fable-5")
CLAUDE_TIMEOUT = int(os.environ.get("APP_TG_TIMEOUT", "900"))
...
APPEND_SYSTEM = (
    "You are Claude replying to the owner of the company over Telegram. Keep answers "
    ...
    "the company product/pricing facts live in knowledge-base/products/ (e.g. price-list.md, "
    ...
    "PRIVACY — personal notes: everything under the company/personal/ (files + notes.db) is "
    ...
    "chat — including all the company groups and other users' DMs — behave as if personal/ "
...
# CLIP server + cached Telegram file_ids — sub-second, no LLM. APP_PHOTO_REFLEX=0 off.
PHOTO_REFLEX = os.environ.get("APP_PHOTO_REFLEX", "1") == "1"
# Doc reflex (2026-07-10): curated documents (doc_registry.json) sent instantly via
# sendDocument + cached file_ids — no LLM. APP_DOC_REFLEX=0 off.
DOC_REFLEX = os.environ.get("APP_DOC_REFLEX", "1") == "1"
...
# KB image set, or (DM chats only) the closest-matching the company workspace file. Strict
# all-tokens-match; anything ambiguous falls through. APP_FILE_REFLEX=0 off.
FILE_REFLEX = os.environ.get("APP_FILE_REFLEX", "1") == "1"
...
KB = os.path.join(APP_ROOT, "email", "kb", "kb")   # `kb ask "<question>" --json`
# OFF by default (the owner, 2026-07-07): Telegram chat is Always Claude again — no Nemotron
# quick answers. Set APP_KB_REFLEX=1 to re-enable.
KB_REFLEX = os.environ.get("APP_KB_REFLEX", "0") == "1"
...
KB_PY = os.path.join(APP_ROOT, "email", "venv", "bin", "python")
KB_ANSWER = os.path.join(APP_ROOT, "email", "kb", "kb_answer.py")  # `kb_answer.py "<q>" --json`
PRIVACY_ROUTE = os.path.join(APP_ROOT, "email", "kb", "privacy_route.py")  # strict public/private router
DOCPIPE = os.path.join(APP_ROOT, "local-ai", "docpipe")
...
PRIVACY_MODE = os.environ.get("APP_PRIVACY_MODE", "targeted")  # off | targeted | strict
...
# "Claude the company Public" group. the owner 2026-07-08: "Claude the company Wise" group too — cloud
...
# the owner 2026-07-07: "Claude the company Private" group. NOTE: until the DGX Spark lands,
...
# APP_ROOT/projects/<slug>/ — every post (text/voice/photo/doc) is auto-filed there;
...
PIPER = os.path.join(APP_ROOT, "voice", "venv", "bin", "piper")
PIPER_VOICES = {   # one .onnx per language, keyed by ISO-639-1 (whisper's detection)
    "en": os.path.join(APP_ROOT, "voice", "voices", "en_US-lessac-medium.onnx"),
    "ru": os.path.join(APP_ROOT, "voice", "voices", "ru_RU-irina-medium.onnx"),
    "es": os.path.join(APP_ROOT, "voice", "voices", "es_ES-davefx-medium.onnx"),
    "de": os.path.join(APP_ROOT, "voice", "voices", "de_DE-thorsten-medium.onnx"),
    "fr": os.path.join(APP_ROOT, "voice", "voices", "fr_FR-siwis-medium.onnx"),
