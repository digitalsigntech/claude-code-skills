...
# NO KEYWORD INTERCEPTION. Removed for good on 2026-09-05 — the third time
# the owner asked ("I asked you at least three times to remove all keyword
# shortcuts for good. They are very annoying, because they trigger random
# tables when I don't want them. Everything must go through your brain").
# Off-by-default behind an env var was not removal: every new road (the spoken
# path, request 477) forgot the flag and the tables came back. The reflex
# modules remain as TOOLS the model turn calls (each has a CLI); nothing here
# reads a message for trigger words any more. Photos with no caption are still
# looked at for documents (scan_reflex) — that is a file shape, not a keyword.
