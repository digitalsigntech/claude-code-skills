...
APP_ROOT = os.path.dirname(os.path.dirname(KB_DIR))
...
CONTACTS_DB = os.path.join(APP_ROOT, "crm", "contacts.db")
...
# ---- file delivery (the owner 2026-07-08: the private chat must be able to hand over
# actual documents — PDFs, invoices, images — not just talk about them). The chat is
# allowlisted and private; sending confidential the company files there is accepted policy.
...
_DENY_PARTS = ("token", "secret", "credential", "password", "bot_token",
               ...
               # the owner's personal notes (2026-07-10) — never surfaced or sent by this
...
# knowledge-base/uploads/ by the gateway at ingest time (the owner 2026-07-24 — "copy
...
def _sendable(path):
    ...
    the company workspace, exists, fits Telegram's upload cap and isn't credential-like."""
    real = os.path.realpath(path if os.path.isabs(path)
                            else os.path.join(APP_ROOT, path))
    if not real.startswith(APP_ROOT + os.sep):
        return None, f"{path} is outside the the company workspace — not sendable."
...
def t_find_files(query, limit=8):
    """Filename search under the the company workspace; ranked by terms matched, newest first."""
...
def t_find_media(query, limit=4):
    ...
    'phd-connect-32-v1.5-a.jpeg' (the owner 2026-07-21, the company Private)."""
    ...
    out = [{"path": os.path.relpath(h["path"], APP_ROOT),
...
def t_send_file(path, caption=""):
    ...
    return (f"OK — {os.path.relpath(real, APP_ROOT)} will be attached to your reply. "
            "Do not paste its contents; just tell the user what you are sending.")


# PDF read/edit (the owner 2026-07-27: "we need to give Nemotron pdf editing capabilities"
...
def t_edit_pdf(path, edits):
    ...
    out = os.path.join(APP_ROOT, "knowledge-base", "uploads", stem + "-edited.pdf")
    ...
    rel = os.path.relpath(out, APP_ROOT)
...
# Web fetch (the owner 2026-07-27: "give Nemotron a web fetch tool" — after it couldn't
...
def t_fetch_image(page_url, match):
    ...
    script = os.path.join(APP_ROOT, "local-ai", "web_image_fetch.py")
    try:
...
def t_save_image_to_kb(path, annotation, tags=""):
    ...
    if not (real.startswith(WEB_FETCH_DIR + os.sep) or real.startswith(APP_ROOT + os.sep)):
    ...
    media = os.path.join(APP_ROOT, "local-ai", "media")
...
# Diagram rendering (the owner 2026-07-20: "I need proper image diagrams in the private
...
DIAGRAMS_DIR = os.path.join(APP_ROOT, "diagrams", "private")
...
# private), so everything saved here stays on-box (the owner, 2026-07-17).
MEMORY_FILE = os.path.join(APP_ROOT, "knowledge-base", "private", "nemotron-memory.md")
_current_sender = "the owner"   # set by run(); recorded next to each saved fact
_current_chat = <owner-chat-id>      # set by run(); reminders fire into this chat (default: the owner's DM)
    ...
    with open(MEMORY_FILE, "a") as f:
        if new:
            f.write("# Nemotron private memory\n\n"
                    "Facts the the company owners asked the private assistant to remember.\n"
                    "PRIVATE (the owner, 2026-07-17): never list this file in public_paths;\n"
...
def t_schedule_reminder(when, text, kind="ping"):
    ...
    sys.path.insert(0, os.path.join(APP_ROOT, "operations", "reminders"))
    import reminders
    now = time.time()
    try:
...
TOOLS = {
    ...
    "save_memory": (t_save_memory, "REMEMBER a fact permanently. Use when the owner or "
                    "the co-owner EXPLICITLY asks to remember/store/save information ('remember "
    ...
    "find_files": (t_find_files, "Search the the company workspace for files by NAME — invoices, "
    ...
    "send_file": (t_send_file, "Attach a file to your reply — the owner receives it in "
                  "Telegram. Private/confidential the company documents are fine in this chat. "
...
def _call_api(messages, key, timeout=45, deadline=None):
    ...
    payload = {"model": OR_MODEL, "temperature": 0.0, "max_tokens": 4000,
               ...
               # agent loop would answer without lookups). the owner 2026-07-23: use every
...
SYSTEM = (
    "You are the PRIVATE assistant of the company (the company), makers of the "
    "Print Head Doctor and Print Head Tester. You run on-box and are trusted with "
    "confidential company data. You are mid-conversation with the company's co-owners the owner "
    "and the co-owner over Telegram; each message is labeled with its actual sender — address "
    ...
    "warmly and briefly like any assistant would ('Thank you, the owner! Glad it "
    ...
    "rolling summaries usually contain the full answer in one fast call (the owner's "
    ...
    "full-text search inside ALL attachments at once, instead of guessing. When the owner asks for an actual "
    "document (a PDF, invoice, price list, report), use find_files to locate it "
    "and send_file to attach it — this chat is private and trusted, so confidential the company "
...
def run(question, history="", sender="the owner", chat_id=None):
    ...
    _current_sender = (sender or "the owner").strip() or "the owner"
    if chat_id:
    ...
    for turn in range(MAX_TURNS):
        ...
        if not calls:
            ...
            # EXCEPT social messages (congrats/thanks/greetings — the owner 2026-07-21): a
