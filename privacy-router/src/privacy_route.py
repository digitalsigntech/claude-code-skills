...
sys.path.insert(0, APP_ROOT)                       # data_labels
sys.path.insert(0, os.path.join(APP_ROOT, "telegram"))  # privacy_router (intent regex)
...
PRIVATE_SYSTEM = (
    ...
    "mid-conversation with the owner (the company co-owner) over Telegram: the recent chat "
    ...
    "the co-owner (the company co-owner) — each is labeled with its sender; address the person who "
...
def _label(source):
    ...
    return data_labels.label_for(os.path.join(APP_ROOT, "knowledge-base", source))
...
def crm_context(text, max_chars=5000):
    ...
    out = []
    try:
        db = sqlite3.connect(os.path.join(APP_ROOT, "crm", "contacts.db"))
...
def answer_private(text, hits, history="", sender="the owner", chat_id=None):
    ...
    try:
        mem_f = os.path.join(APP_ROOT, "knowledge-base", "private", "nemotron-memory.md")
...
def main():
    ...
    ap.add_argument("--sender", default="the owner",
                    help="first name of the person who sent the message (the owner/the co-owner)")
