...
WHY DURATIONS TRAVEL IN THE CLEAR. the owner chose to bill seconds of audio, in
...
PIPER_BIN = os.environ.get(
    "LQ_PIPER_BIN", os.path.expanduser("~/the company/voice/venv/bin/piper"))
PIPER_VOICE = os.environ.get(
    "LQ_PIPER_VOICE",
    os.path.expanduser("~/the company/voice/voices/en_US-lessac-medium.onnx"))
...
def preferred_model_name():
    """The recogniser this machine should run (the owner, 2026-09-04).
...
def selftest(clip=None):
    ...
    if not ok:
        return 1
    clip = clip or os.path.expanduser("~/the company/voice/test_en.wav")
