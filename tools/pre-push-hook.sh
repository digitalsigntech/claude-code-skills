...
# the company/telegram/skill-export-scrub.md for the conventions.
cd "$(git rev-parse --show-toplevel)" || exit 1
# 2026-08-13: branding added. A fresh install greeted its owner as "I'm Claude,
# running on the box (your the company appliance)" — company and host names are identifiers
...
IDENT='<owner-chat-id>|[Vv]ladimir|\bVlad\b|[Gg]alentovsky|vgalentovsky|vladimir_dst|digitalsigntech|marina@|vlad@|[Mm]arina|neo@|\bNeo\b'
HOSTS='$HOME|[Mm]ercury|\bDST\b|APP_|_DST|summitlabel|[Ss]ummit Label|the Mac dev instance|demo-vps|voice-vps|trading-vps|\bsrv1[0-9]{6}\b'
NUMS='-55[0-9]{8}|\b2\.24\.102\.182\b|<voice-vps-ip>'
HITS=$(grep -rIn -E "$IDENT|$HOSTS|$NUMS" --exclude-dir=.git .)
if [ -n "$HITS" ]; then
  echo "PUSH BLOCKED — owner PII found in working tree:" >&2
  echo "$HITS" | head -20 >&2
  echo "Scrub per the company/telegram/skill-export-scrub.md, commit, then push again." >&2
  exit 1
fi
exit 0
