...
| **realtime-voice** | Live spoken conversation from a phone browser: OpenAI gpt-realtime as mouth/ears (WebRTC direct, ~300 ms), Claude as the brain via tool calls — plus inline media search, an in-page camera Claude can see through, and voice-command chat clearing. No public IP needed (outbound tunnel). Also the backend for the Voice Bridge iOS app (QR-login enrolment, auto-connect, spoken hang-up). | [`realtime-voice/`](realtime-voice/) |
| **agent-voice-mode** | Installer runbook that connects any agent to the Voice Bridge iOS app: sets up telegram-gateway + the realtime-voice backend + HTTPS exposure, then enrolls the phone with a single-use login QR sent over Telegram (no password in the QR). | [`agent-voice-mode/`](agent-voice-mode/) |
